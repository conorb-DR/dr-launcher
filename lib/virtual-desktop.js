...
function runScript(args, timeoutMs = 15000) {
  return new Promise((resolve, reject) => {
    ...
    execFile("powershell.exe", fullArgs, {
      ...
      if (err) {
        console.error(`[vdesktop] err.code=${err.code} err.killed=${err.killed} err.signal=${err.signal}`);
        console.error(`[vdesktop] stderr: ${(stderr || "").trim()}`);
        console.error(`[vdesktop] stdout: ${output.slice(0, 500)}`);
      }
      if (!output) {
