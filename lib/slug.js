...
// `${localpart}-${hash10}` — the per-account portion, reused by customerSlug AND
// by the virtual-desktop name so they share one collision-resistant identity.
function accountTag(serverKey, orgId, orgDomain, email) {
  const acct = sanitize(String(email || "").split("@")[0]).slice(0, 24);
  const hash = crypto
    .createHash("sha256")
    .update(`${String(serverKey || "").toLowerCase()}:${orgId || ""}:${String(orgDomain || "").toLowerCase()}:${String(email || "").toLowerCase()}`)
    .digest("hex")
    .slice(0, 10);
  return `${acct}-${hash}`.replace(/^-+/, "");
}
...
function customerSlug(serverKey, orgId, orgDomain, email) {
  ...
  return `${key}-${org}-${accountTag(serverKey, orgId, orgDomain, email)}`
    .replace(/-{2,}/g, "-")
    .replace(/^-+|-+$/g, "");
}

module.exports = { customerSlug, accountTag, sanitize };
