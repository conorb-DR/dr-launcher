...
// Tolerant parse of the JWT `is_support` claim. It's a clean boolean on the wire
// (verified), but accept truthy variants defensively so a format change can't
// silently demote a support account to "user" (which the support-only default
// would then hide).
function parseIsSupport(v) {
...
function execDr(args) {
  // Runs dr without a shell where possible (see lib/run-command.js), so
  // metacharacters in argument values are never parsed by a shell.
  return runCommand.runDr(args, { timeout: 15000 });
}
...
async function getAccountDetails(serverKey, email) {
  ...
  try {
    ...
    return {
      ...
      isSupport: parseIsSupport(claims.is_support),
...
async function probeWhoami(serverKey, email, opts = {}) {
  ...
  // runDr returns { stdout, stderr, exitCode, timedOut, signal } — the same
  // enriched shape this previously built by hand.
  return runCommand.runDr(["whoami", "--server", key, "--account", email, "--json"], { timeout });
}

/**
 * Probe an account's REAL liveness via an authenticated API call.
 *
 * IMPORTANT: `dr whoami` is offline by design (it only decodes the locally
 * stored JWT and never touches the network), so it returns exit 0 even for a
 * dead session and CANNOT detect expiry. A real call performs a pre-flight JWT
 * refresh against the server — that refresh fails ("jwt auto-refresh failed
 * (pre-flight) … refresh failed") exactly when the ~8-day refresh token has
 * died, which is the only authoritative liveness signal. `templates list` is
 * used because every tenant has it and it's read-only; we only care about the
 * pre-flight refresh outcome, not the payload.
 *
 * Returns { stdout, stderr, exitCode, timedOut, signal }.
 */
async function probeLiveness(serverKey, email, opts = {}) {
  const key = validateServer(serverKey);
  const timeout = opts.timeout || 20000;
  return runCommand.runDr(["templates", "list", "--server", key, "--account", email, "--json"], { timeout });
}
