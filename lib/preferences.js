...
function getPreferences() {
  try {
    ...
    if (err.code !== "ENOENT") {
      const backup = PREFS_PATH + ".corrupt." + Date.now();
      try { fs.renameSync(PREFS_PATH, backup); } catch {}
      logger.log("warn", "preferences", `Corrupt preferences.json backed up to ${path.basename(backup)}: ${err.message}`);
      pruneBackups(PREFS_DIR, "preferences.json.corrupt.", 3);
    }
