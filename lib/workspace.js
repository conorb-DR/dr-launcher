...
/**
 * Folder-safe per-(org, account) slug. Delegates to lib/slug.js so the workspace
 * folder and the Chrome profile dir (chrome.js) use identical logic and can't
 * drift. Encodes org AND account identity.
 */
function customerSlug(serverKey, orgId, orgDomain, email) {
  return slug.customerSlug(serverKey, orgId, orgDomain, email);
}
...
function ensureWorkspace(account) {
  const folderSlug = customerSlug(account.serverKey, account.orgId, account.orgDomain, account.email);
  const folderPath = path.join(WORKSPACE_ROOT, folderSlug);
  fs.mkdirSync(folderPath, { recursive: true });
  return { folderPath, slug: folderSlug };
}
...
function buildClaudeCommand(titlePrefix, titleHoldSeconds = 0, initialPrompt = "") {
  const claudeCmd = initialPrompt
    ? `claude "${initialPrompt.replace(/"/g, '\\"')}"`
    : "claude";

  if (!titlePrefix) return claudeCmd;

  const holdSeconds = Math.max(0, Number(titleHoldSeconds || 0));
  const holdCmd = holdSeconds > 0
    ? ` & timeout /t ${holdSeconds} /nobreak >nul`
    : "";
  return `title ${titlePrefix}${holdCmd} & ${claudeCmd}`;
}
 ...
 * @param {string} [opts.initialPrompt] - Initial prompt to pass to Claude CLI (e.g. "Read AGENT_TASK.md and begin")
 */
function launchTerminal(folderPath, opts) {
  ...
  if (hasWindowsTerminal()) {
    const cmdStr = buildClaudeCommand(titlePrefix, titleHoldSeconds, initialPrompt);
  ...
  const cmdStr = buildClaudeCommand(titlePrefix, 0, initialPrompt);
