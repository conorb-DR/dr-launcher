...
// OAuth callback host. MUST match a redirect URI registered in the Azure app
// registration. Defaults to "localhost" (the registered value). To move to
// 127.0.0.1: set DR_LAUNCHER_OAUTH_HOST=127.0.0.1 (or "redirectHost" in
// auth-config.json) AND register http://127.0.0.1:<port>/auth/callback as a
// redirect URI in Azure AD first — otherwise sign-in will fail.
function redirectHost() {
  return process.env.DR_LAUNCHER_OAUTH_HOST || loadConfig().redirectHost || "localhost";
}
...
function startLogin(port) {
  ...
  pendingAuth = app
    ...
    .then((authUrl) => {
      // `start` is a cmd builtin (no .exe), so invoke it via cmd.exe with the
      // URL as a discrete argv entry instead of interpolating into a shell
      // string. The URL is MSAL-generated/trusted; this avoids shell parsing.
      execFile("cmd.exe", ["/c", "start", "", authUrl], { windowsHide: true });
...
// Pure authorization predicate: a user is authorized if present and either a
// dev session or a non-expired token. Used by the requireAuthenticated route
// gate as well as isAuthenticated().
function isUserAuthorized(user) {
  return !!user && (user.devMode === true || !user.tokenExpired);
}

async function isAuthenticated() {
  return isUserAuthorized(await getCurrentUser());
}
