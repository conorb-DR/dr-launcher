...
/**
 * Per-(org, account) slug for the profile directory name — delegates to
 * lib/slug.js so it matches the workspace slug exactly (same helper).
 */
function profileSlug(serverKey, orgId, orgDomain, email) {
  return slug.customerSlug(serverKey, orgId, orgDomain, email);
}

/**
 * Get the user-data-dir path for a customer (isolated per org + account).
 */
function profilePath(serverKey, orgId, orgDomain, email) {
  return path.join(PROFILE_ROOT, profileSlug(serverKey, orgId, orgDomain, email));
}
...
function launchChrome(account) {
  ...
  const userDataDir = profilePath(account.serverKey, account.orgId, account.orgDomain, account.email);
